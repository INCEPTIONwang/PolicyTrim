...
class ManiskillEnv(gym.Env):
    ...
    def _init_metrics(self):
        ...
        self.first_success_step = torch.full(
    ...
    def _record_metrics(self, step_reward, infos):
        ...
        if "success" in infos:
            ...
            new_success = torch.logical_and(
                infos["success"], self.first_success_step < 0
            )
            self.first_success_step[new_success] = self.elapsed_steps[new_success].to(
                torch.int32
            )
        ...
        episode_info["first_success_step"] = self.first_success_step.clone()
        if self.use_fixed_reset_state_ids:
            reset_state_id = self.reset_state_ids.clone().to(torch.int32)
        else:
            reset_state_id = torch.full(
                (self.num_envs,), -1, device=self.device, dtype=torch.int32
            )
        episode_info["task_id"] = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.int32
        )
        episode_info["trial_id"] = reset_state_id.clone()
        episode_info["reset_state_id"] = reset_state_id
